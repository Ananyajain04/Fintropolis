...
function Dropdowngoals() {
  const [isOpen, setIsOpen] = useState(false);
  const [selectedOptions, setSelectedOptions] = useState([]);
  const dropdownRef = useRef(null);

  const options = ['Home', 'Vacation', 'Emergency Funds', 'Debt/Loans','Education','Family'];
  ...
  return (
    <div className="dropdown-container relative w-[100%] bg-[#fff7ff] rounded-lg" ref={dropdownRef}>
      <div className="dropdown-header flex justify-left items-center h-[100%] bg-[#fff7ff] rounded-lg" onClick={handleToggleDropdown}>
        <span className="pl-[10px] text-[#72717B] ">{selectedOptions.length > 0 ? selectedOptions.join(', ') : 'Goals'}</span>
